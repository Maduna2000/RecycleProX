...
export async function buildPoliceRegisterReport(
  ...
  return {
    ...
    columns: [
      ...
      // "Amount (<symbol>)" — the transaction/goods value, not a
      // payment-status figure; matches src/lib/pdf/policeRegister.ts's own
      // label. A purchase can appear here while still pending (not yet
      // paid) or partly loan-funded — this column intentionally doesn't
      // distinguish that, since the register exists to track goods
      // provenance, not cash reconciliation.
      { key: 'totalAmount', label: `Amount (${meta.currencySymbol})`, width: 0.08, align: 'right', format: 'money', excelWidth: 12 },
    ],
