 ...
 * INC VAT derivation (EX VAT is canonical/stored, matching Product.
 * defaultBuyPrice / Purchases — VAT is added on top, never derived by
 * dividing the stored price back out).
 */
...
// ─── INC VAT derivation ───────────────────────────────────────────────────────
describe('incVatPrice', () => {
  it('derives E230.00 INC from E200.00 EX at 15%', () => {
    expect(incVatPrice('200.00').toFixed(2)).toBe('230.00')
  })

  it('rounds to 2 decimal places', () => {
    expect(incVatPrice('86.96').toFixed(2)).toBe('100.00')
  })
})
  ...
  it('persists header and items with tenant, 2 dp prices, and given sort order', async () => {
    ...
    await createPriceList({
      ...
      items: [
        ...
        { productId: null, displayName: 'MIXED STEEL', category: 'Steel', priceExVat: 2.5, sortOrder: 1 },
      ],
    }, 'user-1')
    ...
    const items = (arg.data.items as { create: { displayName: string; category: string; priceExVat: string; sortOrder: number; tenantId: string }[] }).create
    expect(items).toHaveLength(2)
    expect(items[0]).toMatchObject({ tenantId: 'tenant-1', displayName: 'COPPER', category: 'Copper', priceExVat: '200', sortOrder: 0 })
    expect(items[1]).toMatchObject({ displayName: 'MIXED STEEL', category: 'Steel', priceExVat: '2.5', sortOrder: 1 })
  })
...
describe('duplicatePriceList', () => {
  it('copies items, resets the active flag, and re-dates to today', async () => {
    vi.mocked(prisma.priceList.findUniqueOrThrow).mockResolvedValue({
      ...
      items: [
        { productId: 'prod-1', displayName: 'COPPER', category: 'Copper', priceExVat: '200.00', sortOrder: 0 },
      ],
    } as never)
    ...
    expect(items[0]).toMatchObject({ productId: 'prod-1', displayName: 'COPPER', category: 'Copper', priceExVat: '200.00' })
  })
})
