/**
 * VAT264 — Second-Hand Goods Declaration (SARS / SA Second-Hand Goods Act)
...
function drawField(
  ...
  page.drawText(value || '—', { x: x + 4, y: y - 9, size: 9, font: fonts.bold, color: DARK })
}
...
export async function generateVat264(data: Vat264Data): Promise<Uint8Array> {
  ...
  page.drawText('VAT 264', { x: MARGIN, y: PAGE_H - 32, size: 22, font: bold, color: WHITE })
  page.drawText('Declaration for Acquisition of Second-Hand Goods', {
    x: MARGIN, y: PAGE_H - 50, size: 9, font: reg, color: rgb(0.8, 1, 0.8),
  })
  page.drawText('As required by the Value-Added Tax Act No. 89 of 1991', {
  ...
  const dateStr = data.date.toLocaleDateString('en-ZA')
  ...
  // Table header
  page.drawRectangle({ x: MARGIN, y: y - 4, width: COL_W, height: 18, color: GREEN })
  ...
  for (let i = 0; i < data.lines.length; i++) {
    ...
    if (y < MARGIN + 180) break // guard — don't overflow signature area
  ...
  y -= 14
  const totalStr = `R ${new Decimal(data.totalAmount).toFixed(2)}`
  ...
  const declaration =
    ...
    'is true and correct. I confirm that these goods are second-hand goods as defined in the Value-Added Tax Act No. 89 of 1991.'
  ...
  page.drawText('Seller Signature:', { x: MARGIN + 4, y: y - 6, size: 8, font: bold, color: DARK })
  ...
  if (data.signatureBytes) {
    try {
      ...
      page.drawText('(Signature on file)', { x: MARGIN + 8, y: sigBoxY + 24, size: 8, font: reg, color: GRAY })
    }
  } else {
    page.drawText('(No signature captured)', { x: MARGIN + 8, y: sigBoxY + 24, size: 8, font: reg, color: GRAY })
  }

  // Date signed
  page.drawText('Date:', { x: MARGIN + 220, y: y - 6, size: 8, font: bold, color: DARK })
  page.drawRectangle({ x: MARGIN + 220, y: sigBoxY, width: 120, height: 60, color: LGRAY, borderColor: GRAY, borderWidth: 0.5 })
  page.drawText(dateStr, { x: MARGIN + 224, y: sigBoxY + 24, size: 9, font: reg, color: DARK })

  // ── Footer ────────────────────────────────────────────────────────────────────
  const footY = MARGIN + 20
  hRule(page, footY + 10)
  page.drawText(
    'This document is generated in accordance with Section 20(8) of the VAT Act No. 89 of 1991 and the Second-Hand Goods Act No. 6 of 2009.',
