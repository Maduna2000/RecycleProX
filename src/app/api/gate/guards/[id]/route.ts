...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!target || target.role !== 'security_guard') {
  ...
  try {
    ...
    logger.info({ targetUserId: params.id, action: parsed.data.action, adminId: session.user.id }, 'security_guard.patched')
    return NextResponse.json(user)
  } catch (err) {
    logger.error({ err }, 'PATCH /api/gate/guards/[id] failed')
