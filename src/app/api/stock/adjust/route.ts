...
import { z } from 'zod'
import { manualAdjustment, manualCountAdjustment, ProductNotFoundError, NoAdjustmentNeededError } from '@/lib/services/stockService'
...
const AdjustSchema = z.discriminatedUnion('mode', [
  z.object({
    ...
    quantity: qty.refine((v) => parseFloat(v) > 0, 'Quantity must be > 0'),
...
export async function POST(req: NextRequest) {
  ...
  try {
    const movement = await runWithRequestTenant(req, () =>
      parsed.data.mode === 'count'
        ? manualCountAdjustment({ ...parsed.data, createdByUserId: session.user.id })
        : manualAdjustment({ ...parsed.data, createdByUserId: session.user.id })
    )
    return NextResponse.json(movement, { status: 201 })
  } catch (err) {
    if (err instanceof ProductNotFoundError) return NextResponse.json({ error: err.message }, { status: 404 })
    if (err instanceof NoAdjustmentNeededError) return NextResponse.json({ error: err.message }, { status: 422 })
