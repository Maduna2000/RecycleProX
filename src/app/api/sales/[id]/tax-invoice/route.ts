...
export async function GET(
  ...
  try {
    const { sale, settings, doneByUser } = await runWithRequestTenant(req, async () => {
      ...
      return { sale, settings, doneByUser }
    })
    ...
    const pdfBytes = await generateTransactionNote({
      ...
      partyVatNumber: customer?.vatNumber ?? undefined,
      partyRegNumber: customer?.companyRegNumber ?? undefined,
