...
import { FileDown, Pen, CheckCircle, ExternalLink, RotateCcw, ChevronDown, ChevronLeft, ChevronRight, OctagonX, ShieldCheck, Loader2 } from 'lucide-react'
...
// ─── Generate Register tab ─────────────────────────────────────────────────────

export function GenerateRegisterTab() {
  const today = (() => {
    const n = new Date()
...
export function VisitHistoryTab() {
  ...
  const { data: visitsData, isLoading: visitsLoading } =
    useSWR<{ visits: PoliceVisit[]; total: number }>(
      `/api/police-visits?limit=${VISITS_PAGE_SIZE}&offset=${(visitsPage - 1) * VISITS_PAGE_SIZE}`,
      fetcher
    )
